...
if (-not $slotExists) {
    Write-Host "Error: Blue slot does not exist!" -ForegroundColor Red
    exit 1
}

# Get the blue slot URL
$slotUrl = az webapp deployment slot list --resource-group $ResourceGroupName --name $AppServiceName --query "[?name=='blue'].hostNames[0]" -o tsv
Write-Host "Blue slot URL: https://$slotUrl" -ForegroundColor Green

# Check the app settings for Entra ID configuration
Write-Host "Checking Entra ID settings..." -ForegroundColor Cyan
$entraIdSettings = az webapp config appsettings list --resource-group $ResourceGroupName --name $AppServiceName --slot blue --query "[?starts_with(name, 'EntraId__')]" -o table
...
foreach ($setting in $requiredSettings) {
    $exists = az webapp config appsettings list --resource-group $ResourceGroupName --name $AppServiceName --slot blue --query "[?name=='$setting'].name" -o tsv
    if (-not $exists) {
...
if ($missingSettings.Count -gt 0) {
    ...
    foreach ($setting in $missingSettings) {
        Write-Host "az webapp config appsettings set --resource-group $ResourceGroupName --name $AppServiceName --slot blue --settings `"$setting=YOUR_VALUE`"" -ForegroundColor Yellow
    }
} else {
...
$deploymentLogs = az webapp log deployment show --resource-group $ResourceGroupName --name $AppServiceName --slot blue
